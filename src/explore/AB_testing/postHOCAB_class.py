...
# from preprocessing_formatting import drop_cols , one_hot_encoding , scale   #5/20 trying to use the pre_format file
...
class ABmodeling:
    ...
    def temp_test(self):
        ...
        for index,item in enumerate([(6544, ['NA', 6530, 6560]) , (6561, ['NA', 6530, 6560, 6544]), (6585, ['NA', 6530, 6560, 6544])]): 
            if len(item[1]) <=1: #
                ...
                self.test_df = self.df1[self.df1['Health_Camp_ID'] == iD ]  
                self.train_df = self.df2.loc[ self.df2['Health_Camp_ID'].isin(camps)  ]  

                #self.get_result = run_tests(test_df,train_df)
                self.ans[index] =  len(self.test_df) #get_result
    ...
    def run_tests(self ):
        ...
        # self.test_df2 , self.train_df2 = self.test_df.copy() , self.train_df.copy() 
        # self.test_df3 , self.train_df3 = self.test_df.copy() , self.train_df.copy() 
        #test_df4 , train_df4 = test_df.copy() , train_df.copy()

        get_L =  beta.run_test_typeL()
        # get_knn = run_test_typek(self.test_df2 , self.train_df2)
        # get_svc = run_test_typeS(self.test_df3 , self.train_df3)
    ...
    def run_test_typeL(self):
        ...
        run knn for post hoc- analysis 
        '''
        self.train_y = self.train_df1['y_target'].values #getting y_target values for test & train
        # test_y = test_dfl['y_target'].values 

        del self.train_df1['y_target'] #deleting values 
        del self.test_df1['y_target']

        df_train = self.train_df1
        df_test = self.test_df1
